...
LOOK
  sight     main "look": app, page, keyboard state, AT A GLANCE (top
            ...
            one-line version when you just need to check something small.
...
ACT
  ...
  scroll    scroll [times] [up|down]   (down = reveal content below)
            scroll "elem" [times] [up|down]  scroll inside a specific panel
...
Per-command help:  python3 phone.py help <command>
"""
...
swipe [left|right|up|down | X1 Y1 X2 Y2 | "elem" <dir> | "from" "to"] [ms]
  ...
  "from" "to"        - drag from one element to another (drag-and-drop).""",
    "scroll": """\
scroll [times] [up|down] - scroll page. down = finger up (next content).
...
def elements():
    ...
    def add(label, t, d, b, x1, y1, x2, y2, cx, cy, clk, node, cls, web, icon):
        ...
        out.append({
            "text": t, "desc": d, "bounds": b, "clickable": clk,
...
def display_label(e):
    ...
    return e["text"] or e["desc"] or e.get("icon_label") or "(unlabeled)"
...
def cmd_scroll(args):
    ...
    if element:
        ...
        x1, y1, x2, y2 = b
        cx = (x1 + x2) // 2
        pad = max(30, int((y2 - y1) * 0.15))
        if direction == "up":
            sy, ey = y2 - pad, y1 + pad
        else:
            sy, ey = y1 + pad, y2 - pad
        for _ in range(times):
            shell("input", "swipe", str(cx), str(sy), str(cx), str(ey), "120")
            time.sleep(0.3)
        print(f"scrolled {times}x {direction} inside {element!r}")
        return
    w, h = screen_space()
    mx = w // 2
    if direction == "up":
        ...
        y1, y2 = int(h * 0.85), int(h * 0.15)
    for _ in range(times):
        shell("input", "swipe", str(mx), str(y1), str(mx), str(y2), "120")
        time.sleep(0.3)
    print(f"scrolled {times}x {direction}")
...
def cmd_state():
    ...
    locked = is_locked(data)
    lock_str = {True: "yes", False: "no"}.get(locked, "?")
    print(f"device online | {size[0]}x{size[1]} | battery {battery_level(data)}% | "
          f"screen {screen_state(data)} | locked {lock_str} | "
          f"time {data.get('date', '').strip()}")
    vol = "?"
    m = re.search(r"volume is (\d+) in range \[\d+\.\.(\d+)\]",
                  data.get("vol", ""))
    if m:
    ...
    dnd = {"0": "off", "1": "important", "2": "alarms", "3": "block"}.get(
        zen, zen)
    app = focused_app(data)
    print(f"volume {vol} | orientation {orient} | dnd {dnd} | "
          f"wifi {'on' if wifi == '1' else wifi} | "
          f"bluetooth {'on' if bt == '1' else bt}")
    print(f"focused: {app_name(app)} ({app})")
...
def main():
    ...
    if cmd == "home":
        ...
        cmd = "orient"
    elif cmd == "screenshot" or cmd == "screencap":
        cmd = "shot"
    elif cmd == "type":
        cmd = "text"
    elif cmd == "bt":
        ...
        cmd = "mem"
    if cmd == "sight" and args and args[0] == "--brief":
...
def _run(cmd, args, auto):
    ...
    if auto:
        ...
        lines, changed = diff_report(before_els, after_els, before_app,
                                     after_app)
        print("\n===== CHANGED =====")
        if changed:
            for line in lines:
