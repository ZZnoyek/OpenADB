# OpenABD
A platform for AI agents like opencode to interact with your android device through adb (only on Linux) and especially for AIs that do not have machine vision.

# Imortant:
**1. if you want to use openadb with a ai agent Please use OpenCode as that currantly is the only supported agent. (will try to make it support other agents)**
**2. Please have opencode installed from the official website. As you need opencode installed BEFORE installing OpenADB**
**3. The installer will be updated and the actually app that is uploaded in this reop i have to update manually and i WILL NOT leave past versions of the app uploaded.**
**4. This app currantly ONLY supports Linux and Mainly supports OpenCode Tui Version.**

# Updates
**Trying to make a normal explanation that is NOT for the ai and is for human reading. Just in case a normal person wants to use it.**
**Also tryinng to make older apps and integrate a version choser in the installer for backwards compatability.**
