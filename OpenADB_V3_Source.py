...
APP_NAMES = {
    ...
    "com.google.android.apps.calendar": "Calendar",
    ...
    "com.google.android.camera": "Camera",
    "com.android.camera": "Camera",
    "com.google.android.apps.tachyon": "Google Duo",
    "com.google.android.apps.meetings": "Google Meet",
    "com.google.android.apps.translate": "Translate",
    "com.google.android.apps.lens": "Lens",
    "com.google.android.videos": "Play Movies & TV",
    "com.google.android.apps.magazines": "Google News",
    "com.google.android.apps.chromecast.app": "Google Home",
    "com.google.android.apps.giant": "Google Home",
    "com.google.android.companionapp": "Google Home",
    ...
    "com.samsung.android.app.todo": "Samsung Tasks",
    ...
    "com.samsung.android.devicecare": "Device care",
    "com.sec.android.app.shealth": "Samsung Health",
    "com.samsung.android.app.watchmanager": "Galaxy Wearable",
    "com.samsung.smartswitch": "Smart Switch",
    "com.sec.android.app.samsungapps": "Galaxy Store",
    "com.sec.android.app.clock": "Clock",
...
ACT
  ...
  scroll    REAL scroll (not a swipe): flings content like a human flicking
            a list - momentum scrolling. scroll [times] [up|down|left|right]
            (down = reveal content below, left = reveal content to the
            right). scroll "elem" [times] [up|down|left|right]  scroll
  ...
  key       key <home|back|recents|power|volup|voldown|enter|tab|del|
            play|pause|next|prev|copy|paste|cut|...>  (full list: `key`
            with no arg)
  open      open URL in Brave, or launch ANY installed app by its human
            name, package, or fuzzy match (user + system apps):
            open whatsapp | open tiktok | open com.discord | open --list music
  close     force-stop an app (kills it, no confirmation):  close com.whatsapp
  install   install an APK from the PC onto the phone:
            install ./app.apk   (replaces existing + allows test APKs)
  ...
  clipboard get or set clipboard text (best-effort, not supported on all
            devices/Android versions):  clip  |  clip set "hello"
...
DEVICE
  ...
  flashlight flashlight [on|off|state]   (torch alias)
  nfc       nfc [on|off|state]
  location  location [on|off|state]   (gps alias)
  sound     sound [normal|vibrate|silent|state]   (ringer mode)
  net       net [state|info]   (wifi/data state + reachability, or active
            network info)
...
AI WORKFLOW
  ...
  url / sleep / ram / screencap / torch / gps.  Version: -version.
...
swipe [left|right|up|down | X1 Y1 X2 Y2 | "elem" <dir> | "from" "to"] [ms]
  ...
  "from" "to"        - drag from one element to another (drag-and-drop).""",
    "scroll": """\
scroll [times] [up|down|left|right] - REALLY scroll content (a fling with
momentum, like flicking a list), NOT a swipe. down = reveal content below,
left = reveal content to the right (horizontal lists).
scroll "elem" [times] [up|down|left|right] - scroll inside a specific
element/panel.
...
play|pause|next|prev|stop|ff|rew|mute|camera|call|endcall|comma|period|
slash|backslash|semicolon|apostrophe|grave|minus|equals|leftbracket|
rightbracket|plus|at|hash|star|pageup|pagedown|copy|paste|cut""",
    "open": """\
open <url | app name | package> - URL opens in Brave; otherwise launches ANY
installed app (user or system) by human name (whatsapp, brave, settings...),
exact package (com.google.android.youtube), or fuzzy match (tiktok ->
com.zhiliaoapp.musically, netflix -> com.netflix.mediaclient). If several
DIFFERENT apps match, the candidates are listed and `open <exact-package>` is
required.  open --list <name> shows all matches without launching.
Examples: open example.com  /  open whatsapp""",
    "close": """\
...
uninstall <package | app name> - uninstall an app entirely.
  uninstall whatsapp  /  uninstall com.discord""",
    "install": """\
install <apk-path> - install an APK from the PC onto the phone.
  install ./app.apk  /  install ~/Downloads/whatsapp.apk
Uses adb install -r -t (replaces an existing install, and allows test /
debug APKs). Prints 'Success' and exits 0 on success, exits 1 on failure.""",
    "clear": """\
...
Example script:
  ...
  sight --brief""",
    "clipboard": """\
clip [set "<text>"] - print the current clipboard text, or set it (best
effort; `cmd clipboard set-text` is not available on all devices/Android
versions - falls back to a clear message, then use `text` to type).""",
    "notify": """\
notify - list current notification titles + apps.
notify --json - machine-readable list (title/text/pkg).
notify --count - just the number of notifications.""",
    "apps": """\
...
ocr --json [filter] - same as structured JSON (text/center/conf).""",
    "shot": """\
...
data [on|off|state] - toggle or read mobile data.""",
    "flashlight": """\
flashlight [on|off|state] - toggle the camera flash / torch.  (torch alias)""",
    "nfc": """\
nfc [on|off|state] - toggle or read NFC.""",
    "location": """\
location [on|off|state] - toggle location (GPS). on = high accuracy,
off = location off.  (gps alias)""",
    "sound": """\
sound [normal|vibrate|silent|state] - set or read the ringer mode.""",
    "net": """\
net [state|info] - state: wifi / mobile data / internet reachability.
info: active network details.""",
    "uptime": """\
uptime - how long the device has been running.""",
    "signal": """\
...
screen state, locked, battery, keyboard, volume, orientation, rotation,
dnd, wifi, bluetooth, nfc, notification count, uptime, focused field,
elements with bounds/centers/state, and OCR lines with coordinates).
...
openadb open whatsapp              open an app
   openadb install app.apk            install an app file (APK) from your PC
...
USEFUL EXTRAS
   ...
   openadb install app.apk          install an app file (APK) from the PC
  ...
  openadb clip set "this is it"    put text on the phone's clipboard
  openadb flashlight on            turn on the torch
  openadb location on              turn on location (GPS)
  openadb sound silent             mute the phone
  openadb net                      wifi / data / internet status
  openadb uptime                   how long the phone's been on
...
def _popen_poll(cmd, timeout=20, binary=False):
    """Run `cmd` via a SIGINT-safe poll loop.

    subprocess.run/communicate wait in a C-level waitpid loop that swallows
    EINTR, so ^C during a hanging adb call would not abort (the
    KeyboardInterrupt stays pending until the child exits - up to `timeout`s,
    or forever). Polling with select() keeps Ctrl+C responsive, drains the
    pipes so large dumpsys/`top` output can't deadlock, and any still-running
    child is killed before the KeyboardInterrupt is re-raised.
    Returns a CompletedProcess, or None on timeout, or raises
    KeyboardInterrupt (child already cleaned up).
    """
    try:
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE)
    except FileNotFoundError:
        print(f"{cmd[0]} not found on this machine (is it installed and "
              "on PATH?)")
        sys.exit(1)
    ...
    try:
        while True:
            ...
            if time.time() >= deadline:
                ...
                try:
                    ...
                    pass
                return None
            rlist, _, _ = select.select([p.stdout, p.stderr], [], [], 0.05)
            for fd in rlist:
                chunk = os.read(fd.fileno(), 65536)
                if chunk:
                    (buf_out if fd is p.stdout else buf_err).extend(chunk)
        out, err = p.communicate()
        buf_out.extend(out or b"")
        buf_err.extend(err or b"")
        ...
        try:
            ...
            pass
        raise
    out, err = bytes(buf_out), bytes(buf_err)
    if not binary:
...
def adb(*args, timeout=20):
    cp = _popen_poll(["adb", *args], timeout=timeout)
    if cp is None:
...
def cmd_sight(brief=False):
    ...
    mrot = re.search(r"mCurrentRotation=ROTATION_(\d)", data.get("window", ""))
    if mrot and int(mrot.group(1)) in (1, 3):
        w_phys, _ = screen_size() or (1080, 2340)
        h = w_phys
...
def sight_json():
    ...
    data = batch_shell({
        ...
        "battery": "dumpsys battery",
        "trust": "dumpsys trust",
        "notif": "dumpsys notification --noredact",
        "volume": "cmd media_session volume --stream 3 --get",
        "settings": ("echo accel:$(settings get system accelerometer_rotation); "
                     "echo u_rot:$(settings get system user_rotation); "
    ...
    kv = {}
    for line in data.get("settings", "").splitlines():
        m = re.match(r"(accel|u_rot|zen|wifi|bt|nfc):(.*)", line)
        if m:
            v = m.group(2).strip()
            kv[m.group(1)] = "" if v == "null" else v
    ...
    m = re.search(r"mCurrentRotation=ROTATION_(\d)", data.get("window", ""))
    if m:
        rotation_deg = {"0": 0, "1": 90, "2": 180, "3": 270}[m.group(1)]
    else:
        rotation_deg = None
    if kv.get("accel") == "1" and rotation_deg is not None:
        orientation = "auto"
    elif rotation_deg in (0, 180):
    ...
    m = re.search(r"^\s*(\d+)\.\d+\s", data.get("uptime", ""), re.M)
    if m:
        secs = int(m.group(1))
        uptime = f"{secs // 86400}d {secs % 86400 // 3600:02d}:" \
                 f"{secs % 3600 // 60:02d}:{secs % 60:02d}"
    else:
        uptime = None
    zen = {"0": "off", "1": "important only", "2": "alarms only",
           "3": "block all"}.get(kv.get("zen", ""), kv.get("zen") or None)
    ...
    return {
        ...
        "bluetooth": onoff("bt"),
        "nfc": onoff("nfc"),
        "notifications": len(_notify_entries(data.get("notif", ""))),
        "uptime": uptime,
        "focused_field": ({
...
def _wait_element(query, o, prefer_scrollable=False):
    """Resolve an ELEMENT (not a point) honoring --wait/--index; returns
    (el, hits) or exits. Used where the whole element's bounds are needed,
    e.g. swipe/scroll inside a named panel."""
    deadline = time.time() + (o["wait"] if o["wait"] else 0)
    while True:
        els = elements()
        best, hits = _matches(query, els, prefer_scrollable=prefer_scrollable)
        if hits:
            if o["index"] is not None:
                if o["index"] < 0 or o["index"] >= len(hits):
                    print(f"--index {o['index']} out of range: {len(hits)} "
                          f"match(es) for {query!r}")
                    sys.exit(1)
                return hits[o["index"]], hits
            return best, hits
        if not o["wait"] or time.time() >= deadline:
...
def cmd_swipe(args):
    ...
    if len(pos) == 2 and pos[0] not in dirs and pos[1] in dirs:
        ...
        el, _ = _wait_element(pos[0].lower(), o, prefer_scrollable=True)
        b = _bounds_of(el)
        if not b:
...
def cmd_scroll(args):
    ...
    for a in args:
        if a in ("--drag", "--slow"):
            ...
            mode = "fling"
        elif a.lower() in ("up", "down", "left", "right"):
            direction = a.lower()
        else:
            try:
                times = int(a)
    ...
    def do_fling(x1, y1, x2, y2):
        shell("input", "swipe", str(x1), str(y1), str(x2), str(y2), str(dur))
        time.sleep(gap)

    def fling_rect(x1, y1, x2, y2):
        """One directional swipe within the rect (x1,y1)-(x2,y2)."""
        if direction in ("left", "right"):
            pad = max(30, int((x2 - x1) * (0.28 if mode == "fling" else 0.15)))
            cy = (y1 + y2) // 2
            if direction == "left":
                return x2 - pad, cy, x1 + pad, cy
            return x1 + pad, cy, x2 - pad, cy
        pad = max(30, int((y2 - y1) * (0.28 if mode == "fling" else 0.15)))
        cx = (x1 + x2) // 2
        if direction == "up":
            return cx, y1 + pad, cx, y2 - pad
        return cx, y2 - pad, cx, y1 + pad
...
KEYS = {
    ...
    "comma": "55", "period": "56", "slash": "76", "backslash": "73",
    "semicolon": "74", "apostrophe": "75", "grave": "68",
    "minus": "69", "equals": "70", "leftbracket": "71",
    "rightbracket": "72", "plus": "81", "at": "77", "hash": "18",
    "star": "17", "pound": "18", "pageup": "92", "pagedown": "93",
    "copy": "278", "paste": "279", "cut": "277", "softleft": "1",
    "softright": "2", "compose": "121", "brightness_down": "220",
    "brightness_up": "221",
}
...
def _open_url(url):
    ...
    if r.returncode != 0 or re.search(r"\berror\b", out, re.I):
        print(f"could not open {url} in Brave: "
              f"{(out.strip() or 'no response from adb')[:150]} "
              f"(is Brave installed on the phone?)")
        sys.exit(1)
    print("opened in Brave:", url)


def _normalize(s):
    """Lowercase and strip all non-alphanumerics so 'YouTube Music' and
    'com.google.android.apps.youtube.music' both normalize to matching keys."""
    return re.sub(r"[^a-z0-9]", "", s.lower())


def _installed_packages(users_only=False):
    out = shell("pm", "list", "packages", "-3") if users_only \
        else shell("pm", "list", "packages")
    return re.findall(r"package:(.+)", out)


def _resolve_candidates_ranked(target, users_only=False):
    """Ranked (pkg, tier, source) list for a human or package target.

    Searches the KNOWN-NAME table AND every installed app (user + system),
    so `open` can launch any app you can name. Tiers (lower = better):
      0  the exact package the user typed
      1  exact known human name (APP_NAMES)
      2  known human name prefix / fuzzy
      3  installed package or its tail == the query
      4  query is a substring of an installed package
      5  query is a substring of the normalized package
    """
    t = target.strip()
    low = t.lower()
    norm = _normalize(t)
    ...
    def add(pkg, tier, src):
        if not pkg or pkg in seen:
            return
        seen.add(pkg)
        out.append((pkg, tier, src))

    if "." in t:
        add(t, 0, "package")

    named = sorted(APP_NAMES.items(), key=lambda kv: kv[1].lower())
    for pkg, name in named:
        if _normalize(name) == norm:
            add(pkg, 1, "name")
    if not any(o[1] == 1 for o in out):
        for pkg, name in named:
            nm = _normalize(name)
            if nm.startswith(norm) or norm.startswith(nm):
                add(pkg, 2, "name")

    pkgs = _installed_packages(users_only=users_only)
    for pkg in pkgs:
        if _normalize(pkg) == norm or _normalize(pkg.rsplit(".", 1)[-1]) == norm:
            add(pkg, 3, "package")
    if not any(o[1] <= 3 for o in out):
        for pkg in pkgs:
            if low in pkg.lower():
                add(pkg, 4, "package")
    if not any(o[1] <= 4 for o in out):
        for pkg in pkgs:
            if norm in _normalize(pkg):
                add(pkg, 5, "package")
    return out


def _resolve_candidates(target):
    """Package candidates, best first (exact package >> known name > fuzzy).
    `close`/`uninstall`/`clear` stay limited to user apps + known names so a
    fuzzy typo can't force-stop a system app by surprise."""
    return [pkg for pkg, _, _ in _resolve_candidates_ranked(target,
                                                            users_only=True)]
...
def launch_app(pkg):
    """Try the app's launcher activity, then monkey (0-arg launches the
    default activity), then a bare MAIN/LAUNCHER intent. Returns True if a
    ...
    if act:
        ...
        if r.returncode == 0 and "Error" not in r.stdout:
            return True
    r = adb("shell", "monkey", "-p", pkg, "-c",
            "android.intent.category.LAUNCHER", "1")
    if r.returncode == 0 and re.search(r"Events injected", r.stdout):
        return True
    r = adb("shell", "am", "start", "-a", "android.intent.action.MAIN",
...
def _show_candidates(target):
    ranked = _resolve_candidates_ranked(target)
    if not ranked:
        print("no app matched:", target)
        return False
    print(f"{len(ranked)} match(es) for {target!r}:")
    for i, (pkg, tier, src) in enumerate(ranked[:25], 1):
        print(f"  {i}. {pkg}  ({app_name(pkg)})  [{src}]")
    return True


def cmd_open(target):
    if target.startswith("--list"):
        name = target[len("--list"):].strip()
        if not name:
            print("usage: open --list <name>")
            sys.exit(1)
        _show_candidates(name)
        return
    if "://" in target or target.startswith("www."):
        _open_url(target)
        return
    ranked = _resolve_candidates_ranked(target)
    to_try = [pkg for pkg, _, _ in ranked]
    if to_try:
        best_tier = min(o[1] for o in ranked)
        top = [o for o in ranked if o[1] == best_tier]
        exact_pkg = best_tier == 0 and len(top) == 1
        if exact_pkg:
            to_try = [top[0][0]]
        elif len(top) > 1 and len({app_name(pkg) for pkg, _, _ in top}) > 1:
            print(f"several apps match {target!r}:")
            for pkg, tier, src in top[:12]:
                print(f"    {pkg}  ({app_name(pkg)})")
            print(f"be specific: re-run with the exact package, e.g.  "
                  f"open {top[0][0]}")
            sys.exit(1)
    for pkg in to_try:
        if launch_app(pkg):
    ...
    if m and target.count(".") <= 2 \
            and re.match(r"^[a-z]{2,6}$", target.rsplit(".", 1)[-1]):
        _open_url(target)
        return
...
def cmd_install(path):
    if not path:
        print("install <apk-path>   e.g. install ./app.apk")
        sys.exit(1)
    if not os.path.isfile(path):
        print("install: no such file:", path)
        sys.exit(1)
    if not path.lower().endswith(".apk"):
        print(f"install: '{path}' doesn't look like an .apk")
        sys.exit(1)
    print(f"installing {path} ...")
    r = adb("install", "-r", "-t", path, timeout=180)
    out = (r.stdout or "").strip()
    if r.returncode == 0 and "Success" in out:
        print(out)
    else:
        print("install failed:", out or (r.stderr or "").strip())
        sys.exit(1)
...
def screenshot_bytes():
    cp = _popen_poll(["adb", "exec-out", "screencap", "-p"], timeout=20,
                     binary=True)
...
def _tesseract_lines(path, psm):
    cp = _popen_poll(["tesseract", str(path), "stdout", "--psm", psm, "tsv"],
                     timeout=15)
    if cp is None:
        return []
    r = cp
...
def cmd_ocr(args):
    ...
    if as_json:
        args = args[1:]
    filter_ = " ".join(args) or None
    lines = ocr_lines()
    if lines is None:
        print("[]" if as_json else "tesseract not installed")
        return
    if as_json:
        if filter_:
            lines = [d for d in lines
                     if filter_.lower() in d["text"].lower()]
...
def cmd_apps(filter_):
    if filter_ == "--all":
        ...
        out = shell("pm", "list", "packages", "-s")
        pkgs = sorted(re.findall(r"package:(.+)", out))
        print(f"{len(pkgs)} system packages")
    else:
...
def cmd_volume(args):
    if not args or args[0] == "get":
        mv = _media_volume()
        if mv:
            print(f"media volume: {mv[0]} (max {mv[1]})")
        else:
    ...
    if a == "up":
        ...
        if mv and mv[0] == 0:
            ...
            print(f"media volume: mute requested (readback={mv[0] if mv else '?'})")
    elif a == "unmute":
        mv = _media_volume()
        if mv and mv[0] == 0:
            shell("cmd", "media_session", "volume",
                  "--stream", "3", "--set", str(min(10, mv[1])))
        ...
        try:
            ...
            sys.exit(1)
        shell("cmd", "media_session", "volume",
              "--stream", "3", "--set", str(target))
        mv = _media_volume()
        if mv and mv[0] == target:
            print(f"media volume: {target}")
        elif mv:
            print(f"media volume: {mv[0]} (requested {target}, clamped to max {mv[1]})"
                  if target > mv[1] else
                  f"media volume: set requested ({target}, readback={mv[0]})")
        else:
            print(f"media volume: set requested ({target})")
    else:
        print("usage: volume [up|down|mute|unmute|set N|get]")
        sys.exit(1)
...
def cmd_clipboard(args=None):
    ...
    if args and args[0] == "set" and len(args) > 1:
        text = " ".join(args[1:])
        r = adb("shell", "cmd", "clipboard", "set-text", text)
        out = (r.stdout + r.stderr).strip()
        bad = ("No shell command implementation", "Error", "Exception",
               "Unknown command", "cannot", "unable")
        if r.returncode == 0 and not any(b.lower() in out.lower()
                                         for b in bad):
            shown = text[:40] + ("..." if len(text) > 40 else "")
            print(f"clipboard: set ({shown})")
            return
...
def _notify_entries(out=None):
    if out is None:
        out = shell("dumpsys", "notification", "--noredact")
        if not any("NotificationRecord(" in l for l in out.splitlines()):
            plain = shell("dumpsys", "notification")
            if any("NotificationRecord(" in l for l in plain.splitlines()):
                out = plain
    entries = []
    cur = None
    pending = ""
...
def cmd_notify(args=None):
    ...
    if args and args[0] == "--json":
        print(json.dumps(entries[:20], ensure_ascii=False, indent=2))
        return
...
def cmd_wifi(args):
    ...
    if a == "on":
        ...
        if v == "1":
            ...
            print("wifi: enable requested (not confirmed)")
    elif a == "off":
        ...
        if v == "0":
            ...
            print("wifi: disable requested (not confirmed)")
    else:
        v = shell("settings", "get", "global", "wifi_on").strip()
        print("wifi:", "on" if v == "1" else "off" if v == "0" else "unknown")
...
def cmd_bluetooth(args):
    ...
    if a == "on":
        ...
        if v == "1":
            ...
            print("bluetooth: enable requested (not confirmed)")
    elif a == "off":
        ...
        if v == "0":
            ...
            print("bluetooth: disable requested (not confirmed)")
    else:
        v = shell("settings", "get", "global", "bluetooth_on").strip()
        print("bluetooth:", "on" if v == "1" else "off" if v == "0"
              else "unknown")


def _any_match(query, els=None, with_ocr=True):
...
def cmd_verify(args):
    if args and args[0] == "--gone":
        ...
        while time.time() < deadline:
            n += 1
            # OCR every 3rd poll: a screenshot + double-pass is expensive,
            # and UI-state changes usually show up in the tree first.
            if not _any_match(query, with_ocr=(n % 3 == 0)):
...
def cmd_data(args):
    ...
    if a == "on":
        ...
        if v == "1":
            ...
            print(f"mobile data: failed to enable (value={v or '?'})")
    elif a == "off":
        ...
        if v == "0":
            ...
            print(f"mobile data: failed to disable (value={v or '?'})")
    else:
        v = shell("settings", "get", "global", "mobile_data").strip()
        print("mobile data:", "on" if v == "1" else "off" if v == "0"
              else "unknown")


def cmd_uptime():
    m = re.search(r"^\s*(\d+)\.\d+\s", shell("cat", "/proc/uptime"), re.M)
    if not m:
    ...
    mn, s = divmod(rem, 60)
    print(f"uptime: {d}d {h:02d}:{mn:02d}:{s:02d}")


def cmd_flashlight(args):
    a = args[0] if args else "state"
    if a == "on":
        shell("cmd", "torch", "set", "on")
        v = shell("cmd", "torch", "get").strip().lower()
        if v == "on":
            print("flashlight: on")
        else:
            print("flashlight: failed to enable"
                  + (f" [{v[:40]}]" if v else " (torch unsupported?)"))
    elif a == "off":
        shell("cmd", "torch", "set", "off")
        v = shell("cmd", "torch", "get").strip().lower()
        if v == "off":
            print("flashlight: off")
        else:
            print("flashlight: failed to disable"
                  + (f" [{v[:40]}]" if v else ""))
    else:
        out = shell("cmd", "torch", "get").strip()
        if out.lower() in ("on", "off"):
            print("flashlight:", out.lower())
        else:
            print("flashlight: unknown (unsupported on this device?)",
                  f"[{out[:40]}]" if out else "")
...
def cmd_nfc(args):
    ...
    if a == "on":
        ...
        v = shell("settings", "get", "secure", "nfc_on").strip()
        if v == "1":
            print("nfc: on")
        else:
            print("nfc: enable requested (not confirmed)")
    elif a == "off":
        ...
        if v == "0":
            ...
            print("nfc: disable requested (not confirmed)")
    else:
        v = shell("settings", "get", "secure", "nfc_on").strip()
        if v == "1":
            print("nfc: on")
...
def cmd_location(args):
    a = args[0] if args else "state"
    modes = {"0": "off", "1": "gps only", "2": "battery saving",
             "3": "high accuracy"}
    if a == "on":
        shell("settings", "put", "secure", "location_mode", "3")
        v = shell("settings", "get", "secure", "location_mode").strip()
        if v == "3":
            print("location: high accuracy")
        else:
            print(f"location: failed to enable (mode={v or '?'})")
    elif a == "off":
        ...
        if v == "0":
            ...
            print(f"location: failed to disable (mode={v or '?'})")
    else:
        v = shell("settings", "get", "secure", "location_mode").strip()
        print("location:", modes.get(v, v or "unknown"))


def cmd_sound(args):
    a = args[0] if args else "state"
    modes = {"0": "silent", "1": "vibrate", "2": "normal"}
    if a in ("normal", "silent", "vibrate"):
        code = next(k for k, v in modes.items() if v == a)
        shell("cmd", "audio", "set-ringer-mode", code)
        v = shell("cmd", "audio", "get-ringer-mode").strip()
        if v == code:
            print("sound:", a)
        else:
            print(f"sound: failed to set ({a}, readback={modes.get(v, v or '?')})")
    else:
        out = shell("cmd", "audio", "get-ringer-mode").strip()
        print("sound:", modes.get(out, out or "unknown"))


def cmd_net(args):
    a = args[0] if args else "state"
    if a in ("info", "details"):
        m = re.search(r"Active default network:\s*(\S+)",
                      shell("dumpsys", "connectivity"))
        print("active network:", m.group(1) if m else "unknown")
        return
    wifi = shell("settings", "get", "global", "wifi_on").strip()
    data = shell("settings", "get", "global", "mobile_data").strip()
    r = adb("shell", "ping", "-c", "1", "-W", "2", "8.8.8.8", timeout=10)
    m = re.search(r"(\d+)\s+packets? received|(\d+) received", r.stdout)
    reachable = bool(m) and ((m.group(1) or m.group(2)) != "0") \
        and not re.search(r"100% packet loss", r.stdout)
    print("wifi:", "on" if wifi == "1" else "off" if wifi == "0" else "unknown")
    print("mobile data:", "on" if data == "1" else "off" if data == "0"
          else "unknown")
    print("internet:", "reachable" if reachable else "not reachable")
...
def dispatch(cmd, args):
    if cmd == "status":
        ...
        cmd_uninstall(" ".join(args))
    elif cmd == "install":
        cmd_install(" ".join(args))
    elif cmd == "clear":
        cmd_clear(" ".join(args))
    elif cmd == "run":
        cmd_run(args)
