...
install_deps() {
    ...
    case "$(command -v dnf || echo)" in
        /*) [ -n "$pkgmgr" ] || pkgmgr="dnf"
            ...
            [ -n "$missing" ] && missing=" $(echo $missing | sed 's/ adb/ android-tools/')" ;;
    esac
    case "$(command -v pacman || echo)" in
        /*) [ -n "$pkgmgr" ] || pkgmgr="pacman"
            for b in adb tesseract xdg-open; do
                command -v "$b" >/dev/null 2>&1 || missing="$missing $b"
            done
            [ -n "$missing" ] && missing=" $(echo $missing | sed 's/ adb/ android-tools/')" ;;
    ...
    echo ">> the following packages need to be installed:$missing"
    printf ">> install them now? (requires sudo) [Y/n] "
    read -r ans || ans=""
    case "$ans" in
        [nN] | [nN][oO]) return 1 ;;
    esac

    # Cache sudo credentials once so subsequent commands don't re-prompt.
    echo ">> requesting sudo access..."
    sudo -v || { echo "error: sudo is required to install dependencies" >&2; return 1; }
